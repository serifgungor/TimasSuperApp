...
url = "https://www.timas.com.tr/kitaplar-ve-setler?q=Karakasvet"
...
try:
    ...
    with urllib.request.urlopen(req, timeout=10, context=ssl_context) as response:
        ...
        if m:
            ...
            for p in products:
                title = p.get('title')
                if "Karakasvet" in title or "Shadowghast" in title:
                    print(f"  * Match - Title: {title}, image: {p['image']['src']}")
                else:
                    print(f"  - Title: {title}, image: {p['image']['src']}")
